...
@Component({
    moduleId: module.id,
    selector: 'transaction-type-tab',
    template: `<criteria-form-options tabName="TT" (onChangeDropdown)="getDropdownValues($event)"></criteria-form-options>
                <div><input style="vertical-align:sub;" type="checkbox" (click)="clickApproveAll($event)" [(ngModel)]="approve" [disabled]="!approveAll"><label for="approve">&nbsp;&nbsp;Approve All</label></div>
                  <div class="juGrid" [options]="transactionTypesGridOptions"></div>
                  <div> {{selectedProfileText}}{{baseProfileId}}</div>`,
    encapsulation: ViewEncapsulation.None
})

export class TransactionTypesTabComponent implements OnInit, OnDestroy {

    private transactionTypesGridOptions: GridOptions;
    ...
    ngOnInit() {
        this.service.transactionTypesRef = this;
        this.initTransactionTypesGridOptions();
        this.ofSetUpPageType(CONSTANTS.TT, true);
        this.subscriptionList.push(this.service.findHandler.subscribe(_ => {
            if (_ === CONSTANTS.TT) {
                //console.log('in Transaction types: ',_);
                this.findFunc();
            }
        }));
        ...
        this.subscriptionList.push(this.service.resetGridHandler.subscribe(_ => {
            ...
            this.transactionTypesGridOptions.api.grid.empty();
        }));
    }
    ...
    private getDropdownValues(model: any) {
        //console.log('in transaction types : ', model);
    ...
    private async ueFind() {
        ...
        if (llBaseProfileId !== this.ilBaseProfileId || !this.ibIsTemplate || this.calculationType !== this.dropdownValues['calculationType']) {
            ...
            this.transactionTypesGridOptions.api.pager.refresh();
        }
    ...
    private setUpPage(coId: number, basePrfl: number) {
        ...
        if (lbEnable) {
            this.ofSetUpdateable(true);
            this.transactionTypesGridOptions['isGridEnabled'] = true;   //should be commented out
            lsProtect = '0';
        }
        else {
            this.ofSetUpdateable(false);
            this.transactionTypesGridOptions['isGridEnabled'] = false;    // should be commented out
            lsProtect = '1';
        }

        this.approveAll = this.transactionTypesGridOptions.api.grid.getData().length > 0 && lbEnable === true;
        this.transactionTypesGridOptions['isApprovedAll'] = this.approveAll;
    ...
    private ofSetEnable(abEnable: boolean) {
        if (abEnable) {
            this.transactionTypesGridOptions['isGridEnabled'] = true;
            this.approveAll = true;
        }
        else {
            this.transactionTypesGridOptions['isGridEnabled'] = false;
            this.approveAll = false;
        }
        this.transactionTypesGridOptions['isApprovedAll'] = this.approveAll;
    }
    ...
    public ofSetApprovalStatus(aiParam: number) {
        ...
        const data: any[] = this.transactionTypesGridOptions.api.grid.getData();
        if (aiParam === 1) {
            for (let i = 0; i < data.length; i++) {
                if (data[i].APPRVD_IND === CONSTANTS.NO) {
                    ...
                    li_mod_cnt++;
                    this.saveTTGridData(data[i].ROW_ID, data[i].APPRVD_IND, moment(data[i].APPRVD_DT).format('MM/DD/YYYY'));
    ...
    public saveTTGridData(rowId: string, approvedInd: string, approvedDate: string, comment: string = '') {
        this.service.isUpdatesPending = true;
        this.appService.isSavedPending = this.service.isUpdatesPending;
        const url = `${this.service.pkgRootUrl}P_TRANS_TYP_CHG_U/json?i_rowid=${rowId}&i_apprvd_ind=${approvedInd}&i_apprvd_dt=${approvedDate}&i_apprvd_by=${this.service.currentUserName}&i_cmt_txt=${comment}`;
    ...
    private loadTransactionTypesGridData(params: any) {
        ...
        let url = `${this.service.pkgRootUrl}P_TRANS_TYP_CHG_S/json?i_al_profile_id=${this.baseProfileId}&i_pageNumber=${pageNo}&i_pageSize=${params.pageSize}&i_sortingColumn=${params.i_colName}&i_orderBy=${params.i_orderType}`;
    ...
    private initTransactionTypesGridOptions() {
        return this.transactionTypesGridOptions = {
            pageSize: 10000, noPager: true, quickSearch: false,
            title: 'Transaction Types Changes',
            isGridEnabled: true,
            isApprovedAll: this.approveAll,
            sspFn: this.loadTransactionTypesGridData.bind(this),
            ...
            columnDefs: [
                ...
                { headerName: 'Transaction Type Cd', field: 'TRANS_TYP_CD', sort: true },
                { headerName: 'Change Type', field: 'CHNG_TYP', sort: true },
                { headerName: 'Change Field', field: 'CHNG_FLD', sort: true },
                { headerName: 'Begin Date', field: `BEGN_DT`, sort: true, cellRenderer: row => row.BEGN_DT == null ? '00/00/0000' : moment(row.BEGN_DT).format('MM/DD/YYYY') },
                { headerName: 'Old Value Description', width: 150, field: 'OLD_VALUE_DESCR', sort: true },
                { headerName: 'New Value Description', field: 'NEW_VALUE_DESCR', sort : true },
                { headerName: 'Comment', field: 'CMT_TXT', sort: true, width: 166, exp: `<input type="text" style="color:#000000" (keyup)="config.clickCommentBox(row,$event)" (blur)="config.updateComment(row,$event)" [(ngModel)]="row.CMT_TXT" [disabled]="!config.isApprovedAll" />` },
                { headerName: 'Change Date', field: `CHNG_DT`, sort: true, cellRenderer: row => row.BEGN_DT == null ? '00/00/0000' : moment(row.BEGN_DT).format('MM/DD/YYYY') },
                { headerName: 'Changed By', field: 'CHNG_BY', sort: true },
            ...
            checkApprovalStatus: (row: any, e: any) => {
                ...
                this.service.ofSetSave(true);
                this.saveTTGridData(row.ROW_ID, e.target.checked ? 'Y' : 'N', moment(Date.now()).format('MM/DD/YYYY'), row.CMT_TXT);
            },
