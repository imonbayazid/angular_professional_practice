...
export class ConfirmOkDialog implements OnInit, OnChanges {
    ...
    private constructForm() {
        this.formOptions = {
            ...
            buttons: {
                'OK': { type: 'button', cssClass: 'btn btn-primary', click: () => { this.form.showModal(false); this.value = 1; this.yesCallback(); } },
    ...
    public showOkDialogPromise(title: string, message: string): Promise<Number> {
        this.value = -1;
        if (title)
            this.formOptions['title'] = title;
        this.formOptions['message'] = message;
